...
MIDI_CSV   = "midi_prob.csv"   # ganti kalau nama file kamu berbeda
...
def evaluate_probs(y_true, probs, name="model"):
    """
    y_true: (N,)
    probs: (N, 5) -> Cluster 1..5
    """
    y_pred = probs.argmax(axis=1) + 1  # label 1..5
...
def confusion_plot(y_true, y_pred, title="Confusion matrix"):
    ...
    ax.set_xticklabels([f"C{i}" for i in LABELS])
    ax.set_yticklabels([f"C{i}" for i in LABELS])
...
def main():
    ...
    # -----------------------------
    # Unimodal (langsung dari masing-masing CSV)
    # -----------------------------
    ...
    # Untuk fusion, kita perlu versi dengan prefix kolom supaya merge aman
    ...
    # Helper untuk evaluasi di setiap subset fusion
    ...
    # -----------------------------
    # Confusion matrix baseline & best (kalau mungkin)
    # -----------------------------
    # Untuk kesederhanaan, kita ambil:
    # - baseline: unimodal full
    # - best: jika salah satu fusion subset-based, pakai subset y_true / y_pred-nya

    # Ambil prediksi baseline (full)
    baseline_pred_map = {
    ...
    # Kita cek manual berdasarkan nama
    ...
    if "Audio+Lyrics+MIDI (F1)" in best_name and res_alm is not None:
        confusion_for_fusion("Audio+Lyrics+MIDI", res_alm, "F1")
        print(classification_report(res_alm["y_true"], res_alm["w"][2],
              labels=LABELS, target_names=EMOTION_CLASSES, zero_division=0))
    elif "Audio+Lyrics+MIDI (equal)" in best_name and res_alm is not None:
        confusion_for_fusion("Audio+Lyrics+MIDI", res_alm, "equal")
    elif "Audio+Lyrics (F1)" in best_name and res_al is not None:
        confusion_for_fusion("Audio+Lyrics", res_al, "F1")
    elif "Audio+Lyrics (equal)" in best_name and res_al is not None:
        confusion_for_fusion("Audio+Lyrics", res_al, "equal")
    elif "Audio+MIDI (F1)" in best_name and res_am is not None:
        confusion_for_fusion("Audio+MIDI", res_am, "F1")
    elif "Audio+MIDI (equal)" in best_name and res_am is not None:
        confusion_for_fusion("Audio+MIDI", res_am, "equal")
    elif "Lyrics+MIDI (F1)" in best_name and res_lm is not None:
        confusion_for_fusion("Lyrics+MIDI", res_lm, "F1")
    elif "Lyrics+MIDI (equal)" in best_name and res_lm is not None:
